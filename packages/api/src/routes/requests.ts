...
const createRequestRoute = createRoute({
  ...
  responses: {
    ...
    409: {
      description: 'Duplicate request - an active request with these parameters already exists',
      content: {
        'application/json': {
          schema: errorResponseSchema,
        },
...
const listRequestsRoute = createRoute({
  ...
  request: {
    query: z.object({
      status: z.enum(['active', 'fulfilled', 'cancelled']).optional().describe('Filter by status'),
    }),
  },
  responses: {
    200: {
